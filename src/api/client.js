...
export const api = {
  ...
  complete:      sid         => fetch(`/api/session/${sid}/complete-public`, { method:'PUT', credentials: 'include' }).then(r => r.json()).catch(() => ({ ok: false })),
  reopen:        sid         => parseJson(fetch(`/api/session/${sid}/reopen`,  { method:'PUT' })),
  resetSession:  sid         => fetch(`/api/session/${sid}/public`, { method:'DELETE', credentials: 'include' }).then(r => r.json()).catch(() => ({ ok: false })),
